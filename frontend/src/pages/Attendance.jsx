...
export default function Attendance() {
  ...
  async function handleSubmit(ev) {
    ...
    try {
      ...
      if (selectedEmp && String(selectedEmp) === String(form.employeeId)) {
        api.getAttendance(selectedEmp, dateFilter || undefined)
        .then(res => setRecords(res.records || [])).catch(() => {});
      }
    } catch (err) {
  ...
  // API returns snake_case: id, employee_id, full_name
  // const selectedEmpObj = employees.find(e => String(e.id) === String(selectedEmp));
  const selectedEmpObj = employees.find(
    e => String(e.employee_id) === String(selectedEmp)
  );
    ...
    <>
      ...
      <div className="filter-bar fade-up-1">
        ...
        >
          ...
          {employees.map(emp => (
            // API fields: emp.id, emp.full_name, emp.employee_id
        ...
        <table>
          ...
          <tbody>
            {!selectedEmp ? (
                  ...
                  <div className="empty-state">
                    ...
                    <div className="empty-sub">
                      {dateFilter
                        ...
                        : `${selectedEmpObj?.full_name || "This employee"} has no attendance records yet.`}
                    </div>
              ...
              // rec fields depend on your attendance API response
          	  // Handles both snake_case and camelCase just in case
              const empName = rec.full_name || rec.employee_name || rec.employeeName || selectedEmpObj?.full_name || "";
              const empCode = rec.employee_id || rec.employeeId || rec.employee_code || selectedEmpObj?.employee_id || "";
              const recordedAt = rec.created_at || rec.createdAt;
              const isPresent = rec.status === "Present";
              const av = avatarStyle(empName);
              ...
              return (
                <tr key={rec.id || rec._id}>
                  ...
                  <td>
                    <div className="employee-cell">
                      ...
                      <div>
                        ...
                        <div className="employee-id">{empCode}</div>
                      </div>
                    </div>
                  </td>
                  <td>
                  ...
                  {/* <td style={{ textAlign: "right", fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--text3)" }}>
                    {recordedAt ? format(new Date(recordedAt), "h:mm a") : "—"}
                  </td> */}
                </tr>
              );
            })}
